...
def load_clip_to_cpu(cfg):
    ...
    model_path = clip._download(url, "./checkpoints")
...
class CustomCLIP(nn.Module):
    ...
    def forward_text_to_text(self, image_features=None):
        ...
        label = (
            torch.arange(self.prompt_learner.n_cls_all, device=class_text_features.device, dtype=torch.long)
            .unsqueeze(0)
            .expand(class_text_features.size(0), -1)
        )
...
class LASP(TrainerX):
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.LASP.FINETUNE_VIT_LN:
            ...
            param_groups = [
                {"params": group1, "lr": cfg.OPTIM.LR * 0.1},
                {"params": group2},
            ]
